...
/**
 * Gets the resource from the specified URLs.
 *
...
/**
 * Gets the user's Google Calendar with the specified name. A new Calendar will
 * be created if the user does not have a Calendar with the specified name.
 *
...
/**
 * Parses all sources using ical.js. Registers all found timezones with
 * TimezoneService. Creates an Array with all events and adds the event-ids to
 * the provided Array.
 *
...
function parseResponses(responses) {
  ...
  //No need to process calcelled events as they will be added to gcal's trash
  //anyway
  result = result.filter(function (event) {
    try {
      return (
...
/**
 * Creates a Google Calendar Event based on the specified ICALEvent. Will return
 * null if the event has not changed since the last sync. If onlyFutureEvents is
 * set to true: -It will return null if the event has already taken place. -Past
 * instances of recurring events will be removed
 *
 * @param {ICAL.Component} event - The event to process
 * @param {string} calendarTz - The timezone of the target calendar
 * @return {?Calendar.Event} The Calendar.Event that will be added to the target
 * calendar
 */
function createEvent(event, calendarTz) {
  ...
  if (icalEvent.startDate.isDate) {
    ...
    if (icalEvent.startDate.compare(icalEvent.endDate) == 0) {
      //Adjust dtend in case dtstart equals dtend as this is not valid for
      //allday events
  ...
  if (event.hasProperty("sequence")) {
    //newEvent.sequence = icalEvent.sequence; Currently disabled as it is
    //causing issues with recurrence exceptions
  }
...
/**
 * Checks if the provided event has taken place in the past. Removes all past
 * instances of the provided icalEvent object.
 *
 * @param {ICAL.Component} event - The event to process
...
/**
 * Patches an existing event instance with the provided Calendar.Event. The
 * instance that needs to be updated is identified by the recurrence-id of the
 * provided event.
 *
...
/**
 * Deletes all events from the target calendar that no longer exist in the
 * source calendars. If onlyFutureEvents is set to true, events that have taken
 * place since the last sync are also removed.
 */
...
function processTasks(responses) {
  ...
  Logger.log(
    "Fetched " + existingTasks.length + " existing tasks from " + taskList.title
  );
  ...
  //-------------- Remove old Tasks ----------- ID can't be used as identifier
  // as the API reassignes a random id at task creation
 ...
 * @return {Array.String} Array with all recurrence components found in the
 * provided event
 */
...
/**
 * Parses the provided string to find the name of an Attendee. Will return null
 * if no name is found.
 *
 * @param {string} veventString - The string to parse
 * @return {?String} The Attendee's name found in the string, null if no name
 * was found
 */
...
/**
 * Parses the provided string to find the mail adress of an Attendee. Will
 * return null if no mail adress is found.
 *
 * @param {string} veventString - The string to parse
 * @return {?String} The Attendee's mail adress found in the string, null if
 * nothing was found
 */
...
/**
 * Parses the provided string to find the response of an Attendee. Will return
 * null if no response is found or the response string is not supported by
 * google calendar.
 *
 * @param {string} veventString - The string to parse
 * @return {?String} The Attendee's response found in the string, null if
 * nothing was found or unsupported
 */
...
/**
 * Parses the provided string to find the notification time of an event. Will
 * return 0 by default.
 *
 ...
 * @param {Number} maxRetries - How many times the function should try if it
 * fails
 * @return {?Calendar.Event} The Calendar.Event that was added in the calendar,
 * null if func did not complete successfully
 */
...
/**
 * Checks for a new version of the script at
 * https://github.com/derekantrican/GAS-ICS-Sync/releases. Will notify the user
 * once if a new version was released.
 */
